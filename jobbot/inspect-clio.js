...
async function fillStep2(page){
  ...
  await page.locator('[data-automation-id="add-button"]').click(); await page.waitForTimeout(350); await page.locator('input[name="url"]').first().fill(PORTFOLIO);
}
(async()=>{
  ...
  const success=/application (has been )?(submitted|received)|successfully submitted|thank you for (applying|your application)|thanks for applying|application submitted/i.test(body);
